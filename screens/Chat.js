...
const Chat = ({ navigation, route }) => {
            ...
            headerTitle: () => (
                ...
                >
                    <Avatar
                        rounded
                        source={{
                            uri: messages.length > 0 ? messages[0]?.data.photoURL : undefined,
    ...
    useLayoutEffect(() => {
        if (route.params && route.params.id) {
            const messagesCollectionRef = collection(db, "chats", route.params.id, "messages");
            const messagesQuery = query(messagesCollectionRef, orderBy("timestamp", "desc"));
            const unsubscribe = onSnapshot(messagesQuery, (snapshot) => {
                const messages = snapshot.docs.map((doc) => ({ id: doc.id, data: doc.data() }));
